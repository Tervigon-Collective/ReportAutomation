WITH
    ...
    amz_item_pnl AS (
      SELECT ai.brand_id AS brand_id, ai.report_date AS report_date, ai.amazon_order_id AS amazon_order_id, ai.pnl_refund_status AS pnl_refund_status, ai.product_cost AS product_cost,
        ...
        if(coalesce(ap.payout_basis, 'NONE') = 'NONE' AND ai.pnl_refund_status = 'CANCELLATION', toFloat64(0), if(NOT empty(ap.amazon_order_id) AND coalesce(og.order_gross_total, 0) > 0, coalesce(ap.gross_revenue, ai.item_gross) * (ai.item_gross / og.order_gross_total), ai.item_gross)) AS item_revenue,
        if(coalesce(ap.payout_basis, 'NONE') = 'NONE' AND ai.pnl_refund_status = 'CANCELLATION', toFloat64(0), if(NOT empty(ap.amazon_order_id) AND coalesce(og.order_gross_total, 0) > 0, coalesce(ap.refund_amount, 0) * (ai.item_gross / og.order_gross_total), toFloat64(0))) AS item_refunds,
        if(coalesce(ap.payout_basis, 'NONE') = 'NONE' AND ai.pnl_refund_status = 'CANCELLATION', toFloat64(0), if(NOT empty(ap.amazon_order_id) AND coalesce(og.order_gross_total, 0) > 0, coalesce(ap.commission, 0) * (ai.item_gross / og.order_gross_total), toFloat64(0))) AS item_commission,
        if(coalesce(ap.payout_basis, 'NONE') = 'NONE' AND ai.pnl_refund_status = 'CANCELLATION', toFloat64(0), if(NOT empty(ap.amazon_order_id) AND coalesce(og.order_gross_total, 0) > 0, coalesce(ap.closing, 0) * (ai.item_gross / og.order_gross_total), toFloat64(0))) AS item_closing,
        if(coalesce(ap.payout_basis, 'NONE') = 'NONE' AND ai.pnl_refund_status = 'CANCELLATION', toFloat64(0), if(NOT empty(ap.amazon_order_id) AND coalesce(og.order_gross_total, 0) > 0, coalesce(ap.shipping, 0) * (ai.item_gross / og.order_gross_total), toFloat64(0))) AS item_shipping,
        if(coalesce(ap.payout_basis, 'NONE') = 'NONE' AND ai.pnl_refund_status = 'CANCELLATION', toFloat64(0), if(NOT empty(ap.amazon_order_id) AND coalesce(og.order_gross_total, 0) > 0, coalesce(ap.tax_withheld, 0) * (ai.item_gross / og.order_gross_total), toFloat64(0))) AS item_tax_withheld,
        if(coalesce(ap.payout_basis, 'NONE') = 'NONE' AND ai.pnl_refund_status = 'CANCELLATION', toFloat64(0), if(NOT empty(ap.amazon_order_id) AND coalesce(og.order_gross_total, 0) > 0, coalesce(ap.other_fees, 0) * (ai.item_gross / og.order_gross_total), toFloat64(0))) AS item_other_fees
