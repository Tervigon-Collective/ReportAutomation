...
def generate_pdf_report(metrics, today, timestamp_str, timeframe_start=None, timeframe_end=None, report_type='daily'):
    ...
    try:
        ...
        if isinstance(_t, dict):
            from dashboard_stats import fetch_gross_aov
            _aov_day = today.strftime('%Y-%m-%d') if hasattr(today, 'strftime') else str(today)[:10]
            _aov_brand = int(os.getenv("CLICKHOUSE_BRAND_ID", "20"))
            _t["aov"] = fetch_gross_aov(_aov_brand, start_str or _aov_day, end_str or _aov_day)
