...
COMMENT ON COLUMN public.customer_details_th.bluedart_service_available
    IS 'Boolean indicating if BlueDart service is available for this order type (COD/prepaid). NULL = not checked, TRUE = available, FALSE = not available';

COMMENT ON COLUMN public.customer_details_th.outstanding_balance
    IS 'Outstanding balance amount from uploaded CSV/Excel files';
...
ALTER TABLE IF EXISTS public.shopify_orders
    ADD CONSTRAINT fk_shopify_orders_customer_details FOREIGN KEY (order_id)
    REFERENCES public.customer_details_th (order_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;
CREATE INDEX IF NOT EXISTS orders_pkey
    ON public.shopify_orders(order_id);


ALTER TABLE IF EXISTS public.shopify_order_customer_journey_moments
    ADD CONSTRAINT shopify_order_customer_journey_moments_order_id_fkey FOREIGN KEY (order_id)
    REFERENCES public.shopify_orders (order_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION;


ALTER TABLE IF EXISTS public.shopify_order_line_items
    ADD CONSTRAINT fk_variant FOREIGN KEY (variant_id)
    REFERENCES public.shopify_product_variants (variant_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE SET NULL;


ALTER TABLE IF EXISTS public.shopify_order_line_items
    ADD CONSTRAINT shopify_order_line_items_order_id_fkey FOREIGN KEY (order_id)
    REFERENCES public.shopify_orders (order_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION;


ALTER TABLE IF EXISTS public.product_variant_cost_history
    ADD CONSTRAINT product_variant_cost_history_variant_id_fkey FOREIGN KEY (variant_id)
    REFERENCES public.shopify_product_variants (variant_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION;
CREATE INDEX IF NOT EXISTS idx_product_variant_cost_history_variant_id
    ON public.product_variant_cost_history(variant_id);


ALTER TABLE IF EXISTS public.shopify_line_item_tax_lines
    ADD CONSTRAINT shopify_line_item_tax_lines_item_id_fkey FOREIGN KEY (item_id)
    REFERENCES public.shopify_order_line_items (item_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION;
